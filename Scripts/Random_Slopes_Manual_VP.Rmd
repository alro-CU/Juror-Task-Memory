---
title: "Random_Intercepts_Manual_VP"
author: "Alex Rogers"
date: "2025-09-08"
output: pdf_document
---
...
## Mixed Effects model of case strength predicted by explicit memory and perceieved realism - INCLUDING RANDOM SLOPES + INTERCEPTS

```{r lmm}
#set random seed
set.seed(42) 
# linear mixed effects model - no correlations between slopes
r_case <-lmer(z_case_strength ~ z_episodic + z_semantic + z_realism + (1 + z_episodic + z_semantic + z_realism || num_uid) + (1|scenario), data=bard_z)

#view results
summary(r_case)

#view r2 of model 
full_r2 <- r.squaredGLMM(r_case)
```

## MANUAL Variance Partitioning - Marginal R2

```{r}
### Unique contribution of episodic ### 
# model EPISODIC REMOVED
reduced_no_ep <- lmer(z_case_strength ~ z_semantic + z_realism + (1 + z_semantic + z_realism ||num_uid) + (1|scenario), data=bard_z)
...
reduced_no_sem <- lmer(z_case_strength ~ z_episodic + z_realism + (1 + z_episodic + z_realism ||num_uid) + (1|scenario), data=bard_z)
...
reduced_no_real <- lmer(z_case_strength ~ z_episodic + z_semantic + (1 + z_episodic + z_semantic ||num_uid) + (1|scenario), data=bard_z)
...
reduced_no_ep_sem <- lmer(z_case_strength ~ z_realism + (1 + z_realism ||num_uid) + (1|scenario), data=bard_z)
...
reduced_no_ep_real <- lmer(z_case_strength ~ z_semantic + (1 + z_semantic ||num_uid) + (1|scenario), data=bard_z)
#summary(reduced_no_ep_real)
#view r2 of model EPISODIC REALISM REMOVED
no_ep_real <- r.squaredGLMM(reduced_no_ep_real)
# difference between full model and reduced
episodic_realism <- full_r2[1] - no_ep_sem[1]


# MODEL SEMANTIC REALISM REMOVED 

reduced_no_sem_real <- lmer(z_case_strength ~ z_episodic + (1 + z_episodic ||num_uid) + (1|scenario), data=bard_z)
